                    ...
                    int calc_map, int mjpeg_port, int show_imgs)
{
    ...
    char *backup_directory = option_find_str(options, "yolov2.weights", "/yolov2.weights/");
    ...
    while (get_current_batch(net) < net.max_batches)
    {
        ...
        */
        ...
        draw_train_loss(windows_name, img, img_size, avg_loss, max_img_loss, i, net.max_batches, mean_average_precision,
                        draw_precision, "mAP%", dont_show, mjpeg_port);
#endif    // OPENCV
...
{
    /*从文件中读取数据集中所有label的名称, 存入names数组*/
    list *options = read_data_cfg(datacfg);  // 构造options链表
    char *name_list = option_find_str(options, "names", "../data/names.list");  // coco.data中, 这个属性的值是 data/coco.names
    int names_size = 0;
    char **names = get_labels_custom(name_list, &names_size); //get_labels(name_list);

    /*将data/labels目录下的图片加载到二维数组中*/
...
void run_detector(int argc, char **argv)
{
    ...
    if (gpu_list)
    {
        ...
        for (i = 0; i < len; ++i)
        {
            if (gpu_list[i] == ',') ++ngpus;  // 每次遇到一个逗号,就增加一个gpu的数量
        }
        gpus = (int *) calloc(ngpus, sizeof(int));  // 分配内存并全部初始化为0, free()函数释放内存
        for (i = 0; i < ngpus; ++i)
        {
            gpus[i] = atoi(gpu_list);  // 把含有,的字符串逐个转换为整数
            gpu_list = strchr(gpu_list, ',') + 1;  // strchr返回第一次出现位置的指针
        }
    } else
    {
        gpu = gpu_index;
    ...
    char *cfg = argv[4];                         // cfg/yolov2.cfg
    char *weights = (argc > 5) ? argv[5] : 0;    // backup/yolov2.weights
    if (weights)
        if (strlen(weights) > 0)
            if (weights[strlen(weights) - 1] == 0x0d) weights[strlen(weights) - 1] = 0;
    char *filename = (argc > 6) ? argv[6] : 0;   // data/person.jpg
