...
{
    list *plist = get_paths(filename);  // 每个label构成一个node, 所有nodes构成一个链表
...
data
load_data_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int classes, int use_flip, int use_blur,
                    int use_mixup,
...
void *load_thread(void *ptr)
{
    ...
    {
        *a.d = load_data_detection(a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.classes, a.flip, a.blur, a.mixup,
                                   a.jitter,
...
load_data_augment(char **paths, int n, int m, char **labels, int k, tree *hierarchy, int use_flip, int min, int max,
                  int w, int h, float angle, float aspect, float hue, float saturation, float exposure, int mixup,
    ...
    if (mixup && rand_int(0, 1))
    {
        ...
        for (i = 0; i < d2.X.rows; ++i)
        {
            ...
            else if (mixup == 3)
            {
                ...
                {
                    d.y.vals[i][j] =
                            d.y.vals[i][j] * s1 + d2.y.vals[i][j] * s2 + d3.y.vals[i][j] * s3 + d4.y.vals[i][j] * s4;
                }
            }
        }
