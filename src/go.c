...
void train_go(char *cfgfile, char *weightfile)
{
    ...
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        ...
        if(get_current_batch(net)%100 == 0){
            char buff[256];
            sprintf(buff, "%s/%s.yolov2.weights",backup_directory,base);
            save_weights(net, buff);
        }
        if(get_current_batch(net)%10000 == 0){
            char buff[256];
            sprintf(buff, "%s/%s_%d.yolov2.weights",backup_directory,base,get_current_batch(net));
            save_weights(net, buff);
        }
    }
...
void valid_go(char *cfgfile, char *weightfile, int multi)
{
    ...
    moves m = load_go_moves("yolov2.weights/go.test");
