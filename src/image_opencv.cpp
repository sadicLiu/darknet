...
extern "C" mat_cv *load_image_mat_cv(const char *filename, int flag)
{
    ...
    try
    {
        ...
        if (mat.empty())
        {
            ...
            if (shrinked_filename.length() > 1024)  // 判断是否因为文件名太长导致读取失败
            {
                shrinked_filename.resize(1024);  // 截取1024个字符
        ...
        // 任何类型的指针都可以相互转换, 在c++里, 这种转换是reinterpret_cast
        // 这里将 Mat* 型指针转换为 void**
...
{
    int flag = cv::IMREAD_UNCHANGED;  // cv::IMREAD_UNCHANGED
                 ...
                 int dont_show)
{
    ...
    try
    {
        ...
        cv::putText(img, "Iteration number", cv::Point(draw_size / 2, img_size - 10), cv::FONT_HERSHEY_COMPLEX_SMALL,
                    0.7, CV_RGB(0, 0, 0), 1, CV_AA);
        char max_batches_buff[100];
        sprintf(max_batches_buff, "in cfg max_batches=%d", max_batches);
        cv::putText(img, max_batches_buff, cv::Point(draw_size - 195, img_size - 10), cv::FONT_HERSHEY_COMPLEX_SMALL,
                    0.7, CV_RGB(0, 0, 0), 1, CV_AA);
