...
void file_error(char *s)
{
    ...
    exit(EXIT_FAILURE);  // Process finished with exit code 1
}
...
// 从文件中读取一行; notes: 如果读到了一行的结尾, 最后一个字符一定是 \n
char *fgetl(FILE *fp)
{
    if (feof(fp)) return 0;  // 判断是否读到了文件尾
    size_t size = 512;
    char *line = (char *) malloc(size * sizeof(char));
    if (!fgets(line, size, fp))  // 从文件中读取一行
    {
        free(line);
        return 0;
    }
    ...
    while ((line[curr - 1] != '\n') && !feof(fp))
    {
        ...
        {
            size *= 2;
            line = (char *) realloc(line, size * sizeof(char));  // 重新分配内存, 不改变原有值, 地址改变
            if (!line)
            {
                printf("%ld\n", size);
                malloc_error();
