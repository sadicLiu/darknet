...
maxpool_layer parse_maxpool(list *options, size_params params)
{
    ...
    maxpool_layer layer = make_maxpool_layer(batch, h, w, c, size, stride_x, stride_y, padding, maxpool_depth,
                                             out_channels, antialiasing, params.train);
    return layer;
}
...
layer parse_shortcut(list *options, size_params params, network net)
{
    ...
    layer s = make_shortcut_layer(batch, index, params.w, params.h, params.c, from.out_w, from.out_h, from.out_c,
                                  assisted_excitation, activation, params.train);
...
{
    list *sections = read_cfg(filename);  // 解析cfg文件, 将网络结构保存到section中
    node *n = sections->front;
    if (!n) error("Config file has no sections");
    network net = make_network(sections->size - 1);  // 初始化network
    net.gpu_index = gpu_index;  // gup_index在dark_cuda.c中定义, 在darknet.c中通过extern的方式赋值
    size_params params;
    ...
    parse_net_options(options, &net);  // 解析配置文件中[net]下的参数

#ifdef GPU
    printf("net.optimized_memory = %d \n", net.optimized_memory);
    if (net.optimized_memory >= 2 && params.train) {  // 如果不在配置文件里设置, optimized_memory默认值是0
    ...
    if (batch > 0) net.batch = batch;  // test时默认是1
    if (time_steps > 0) net.time_steps = time_steps;  // test时默认是1
    ...
    n = n->next;  // 逐个解析每一层网络结构
    int count = 0;
    free_section(s);  // 释放第一个section(链表中第一个node的val属性, 注意node并没有释放)
    ...
    while (n)
    {
        ...
        } else if (lt == MAXPOOL)  // parse
        {
            l = parse_maxpool(options, params);
        } else if (lt == REORG)  // parse
        {
...
list *read_cfg(char *filename)
{
    ...
    char *line;
    int nu = 0;
    list *sections = make_list();  // 链表中的node是section类型
    section *current = 0;
    while ((line = fgetl(file)) != 0)
    {
        ...
        switch (line[0])
        {
            case '[':  // [net], [convolutional], [maxpool]...
                current = (section *) malloc(sizeof(section));
                list_insert(sections, current);  // 将当前section插入sections链表
                current->options = make_list();  // section的options属性也是一个链表
                current->type = line;
                break;
            case '\0':  // 空行
            case '#':   // 注释
            case ';':   // 注释
                ...
                break;
            default:
                if (!read_option(line, current->options))  // 将键值对插入当前section的链表中
                {
